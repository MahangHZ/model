...
def signal_ele(t, temporary, cold_stor, heat_stor, ele_stor, season):  # 储电不够
    chp = CHP(temporary)
    ...
    if demand_ele[t] > elestorage_ele_out + chp.E_out_max:
        ...
        gasturbine_ele_out = demand_ele[t] - elestorage_ele_out
        powergrid_ele_out = 0
...
def signal_heat(t, temporary, cold_stor, heat_stor, season):  # 储电够，储热不够
    chp = CHP(temporary)
    gasturbine = GasTurbine(temporary)
    boiler = Boiler(temporary)
    absorptionchiller = AbsorptionChiller(temporary)
...
def mode_ele_first(t, temporary, cold_stor, heat_stor, ele_stor, season):
    elestorage = EleStorage(temporary)
    heatstorage = HeatStorage(temporary)
    coldstorage = ColdStorage(temporary)
    demand = DemandData()
    ...
    if (signal_e == 0) & (signal_h == 0) & (signal_c == 0):
        ...
        result = signal_ele(t, temporary, cold_stor, heat_stor, ele_stor, season)
    elif (signal_e == 0) & (signal_h == 1):
        result = signal_heat(t, temporary, cold_stor, heat_stor, season)
    else:  # (signal_e == 0) & (signal_H == 0) & (signal_E == 1):
        result = signal_cold(t, temporary, cold_stor, season)
    return result

...
class MultiObjectiveGA:
    def __init__(self, times, mover, number, mode, way_pareto):  # way_pareto=0:正pareto，其他：反pareto
        if mover == 1:
            ...
            for i in range(times):
                ...
                if way_pareto == 0:
                    pareto_again = ParetoSorting(set_of_best, number, mode)
                else:
                    pareto_again = ReverseParetoSorting(set_of_best, number, mode)
...
# times, mover, number, mode, way_pareto
a = MultiObjectiveGA(Constant.calculate_times, 1, 1, 0, 0)

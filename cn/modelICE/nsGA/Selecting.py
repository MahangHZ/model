...
class NextGeneration:
    def __init__(self, new_population, number, mode, way_pareto):
        selecting = Selecting(new_population, number, mode, way_pareto)
...
class Selecting:
    def __init__(self, new_population, number, mode, way_pareto):
        ...
        if way_pareto == 0:
            pareto_sorting = ParetoSorting(new_population, number, mode)
        else:
            pareto_sorting = ReverseParetoSorting(new_population, number, mode)
...
class QueuingForCertainHierarchyTranslationResult:
    def __init__(self, translation_result_to_be_queued, number, mode):
        # hierarchy_to_be_queued是待排序（拥挤度排序）的某帕累托层级，数组
        results_of_objectives = ResultsOfObjectives(translation_result_to_be_queued, number, mode)

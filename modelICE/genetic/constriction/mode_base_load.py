...
def mode_base_load(t, temporary, cold_stor, heat_stor, ele_stor):
    chp = CHP(temporary)
    ...
    if (remainder >= 8) & (remainder <= 19):  # 8:00-20:00
        ...
        if demand.E[t] <= gasturbine_ele_out:  # 电量先由chp提供，再由储能提供
            elestorage_ele_in = gasturbine_ele_out - demand.E[t]
            elestorage_ele_out = 0
            powergrid_ele_out = 0
        elif ((demand.E[t] > gasturbine_ele_out)
              & (demand.E[t] <= gasturbine_ele_out + elestorage.get_E_out_max(ele_stor))):
            elestorage_ele_in = 0
            elestorage_ele_out = gasturbine_ele_out + elestorage.get_E_out_max(ele_stor) - demand.E[t]
            powergrid_ele_out = 0
        else:
            elestorage_ele_in = 0
            elestorage_ele_out = elestorage.get_E_out_max(ele_stor)
            powergrid_ele_out = demand.E[t] - gasturbine_ele_out - elestorage_ele_out
        ...
        if demand.E[t] <= elestorage.get_E_out_max(ele_stor):
            ...
            elestorage_ele_out = elestorage.get_E_out_max(ele_stor)
            powergrid_ele_out = demand.E[t] - elestorage_ele_out

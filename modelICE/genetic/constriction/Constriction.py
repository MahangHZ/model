...
def running_judge(temporary, mode):
    heatstor = 0
    coldstor = 0
    elestor = 0
    cold_stor_list = []
    ...
    for t in range(0, demand.E_sheetnrows - 1, Parameters.delttime):
        ...
        if mode == 0:  # 以冷定热再定电
            ...
            if demand.H[t] > heatstorage.get_H_out_max(heatstor) + chp_heat + gasboiler.nominal:
                ...
                result = mode_cold_first(t, temporary, coldstor, heatstor, elestor)
        else:  # 以电定热再定冷  暂缺base load运行（运行模式已写好，就是还没加进来）
            chp_ele = min(demand.E[t], chp.E_out_max)
            ...
            if ((demand.H[t] > heatstorage.get_H_out_max(heatstor) + chp_heat + gasboiler.nominal)
                ...
                result = mode_ele_first(t, temporary, coldstor, heatstor, elestor)

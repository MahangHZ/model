...
def signal_cold(t, temporary, cold_stor, heat_stor, ele_stor):    # 储冷不够
    ...
    if demand.E[t] <= elestorage.get_E_out_max(ele_stor):   # 储电满足
        ...
        powergrid_ele_out = 0
            ...
            (demand.E[t] <= elestorage.get_E_out_max(ele_stor) + gasturbine_ele_out_users)):
        # 储电 + CHP 满足，多余存入储电
...
def signal_heat(t, temporary, heat_stor, ele_stor):   # 储冷> 冷需求， 储热 < 热需求
    ...
    if demand.E[t] <= elestorage.get_E_out_max(ele_stor):           # 储电
        ...
        powergrid_ele_out = 0
            ...
            (demand.E[t] <= elestorage.get_E_out_max(ele_stor) + gasturbine_ele_out)):
        # 储电+ CHP
        elestorage_ele_out = elestorage.get_E_out_max(ele_stor)
        elestorage_ele_in = gasturbine_ele_out + elestorage.get_E_out_max(ele_stor) - demand.E[t] / Parameters.delttime
        powergrid_ele_out = 0
    else:                        # 储电+ CHP + 电网
        elestorage_ele_out = elestorage.get_E_out_max(ele_stor)
        elestorage_ele_in = 0
        powergrid_ele_out = demand.E[t] - elestorage.get_E_out_max(ele_stor) - gasturbine_ele_out
    result = (coldstorage_cold_in, coldstorage_cold_out, heatstorage_heat_in, heatstorage_heat_out,
              elestorage_ele_in, elestorage_ele_out, absorptionchiller_cold_out, boiler_heat_out, gasturbine_ele_out,
              heatpump_cold_out, gasboiler_heat_out, powergrid_ele_out, gasboiler_fuel, gasturbine_fuel,
              heatpump_powergrid)
...
def signal_ele(t, temporary, ele_stor):
    ...
    elestorage_ele_in = 0
    elestorage_ele_out = elestorage.get_E_out_max(ele_stor)
    if demand.E[t] > (elestorage.get_E_out_max(ele_stor) + chp.E_out_max) * Parameters.delttime:      # 储电+ CHP + 电网
        powergrid_ele_out = demand.E[t] - elestorage.get_E_out_max(ele_stor) - chp.E_out_max
...
def mode_cold_first(t, temporary, cold_stor, heat_stor, ele_stor):
    elestorage = EleStorage(temporary)
    heatstorage = HeatStorage(temporary)
    coldstorage = ColdStorage(temporary)
    demand = DemandData()
    ...
    if (signal_e == 0) & (signal_h == 0) & (signal_c == 0):
        ...
        result = signal_cold(t, temporary, cold_stor, heat_stor, ele_stor)
    elif (signal_c == 0) & (signal_h == 1):
        result = signal_heat(t, temporary, heat_stor, ele_stor)
    else:  # (signal_C == 0) & (signal_H == 0) & (signal_E == 1):
        result = signal_ele(t, temporary, ele_stor)
    return result

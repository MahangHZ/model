...
from cn.modelICE.genetic.objectfunction.ObjectiveFunction import Emissions
from cn.modelICE.genetic.objectfunction.ObjectiveFunction import PrimaryEnergyIndex
from cn.modelICE.genetic.Translation import translation


class ParetoSorting:
    def __init__(self, next_species_result, number, season, mode):
        translation_result = translation(next_species_result)
        self.objectives_result = ResultsOfObjectives(translation_result, number, season, mode)
...
class ResultsOfObjectives:
    def __init__(self, translation_result, number, season, mode):
        self.cost_objective = []
        ...
        for i in range(len(translation_result)):
            ...
            cost_result = TotalCostICE(temporary, number, season, mode)
            emission_result = Emissions(temporary, number, season, mode)
            pei_result = PrimaryEnergyIndex(temporary, number, season, mode)
            self.cost_objective.append(cost_result.whole_life_cost)
            self.emission_objective.append(emission_result.emission_total)
            self.pei_objective.append(pei_result.PER)

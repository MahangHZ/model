...
def calculate(times, mover, number, season, mode):
    ...
    for i in range(times):
        ...
        if mover == 0:
            ...
            output_max_result = OutputMaxICE(translation_result, number, season, mode)
        ...
        if mover == 0:
            ...
            fitness_result = FitnessICE(newpopulation_result, number, season, mode).fitness1
    ...
    economy_index = EconomyIndex(max_powerselection_list[perfect_number], mover, number, season, mode)
...
# times, mover, number, season, mode  \\\season 0:冷模式，1 热模式（无蒸汽） 2 冷热模式（无整齐）3 热模式（有蒸汽） 4 冷热模式（有蒸汽）
calculate(Constant.calculate_times, 1, 1, 0, Constant.mode)
